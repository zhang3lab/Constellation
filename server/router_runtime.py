...
from server.moe_layer_runtime import run_topk_moe_layer
...
def run_real_router_stability_test(session, layer_id: int, repeats: int = 10):
    ...
    for i in range(repeats):
        ...
        print(
            f"[router-stability] iter={i} "
            f"min={combined.min():.6e} max={combined.max():.6e} mean={combined.mean():.6e}"
        )
        ...
        if int(finite.sum()) != combined.size:
            raise RuntimeError("combined output contains non-finite values")
...
def run_real_router_demo(session, layer_id: int, repeats: int = 10):
    ...
    print("[router] combined[:8] =", result["combined"][:8])

    combined = result["combined"]
    finite = np.isfinite(combined)
    finite_count = int(finite.sum())
    total = combined.size
    print(f"[router] combined finite={finite_count}/{total}")
    print(
        f"[router] combined stats: "
        f"min={combined.min():.6e} max={combined.max():.6e} mean={combined.mean():.6e}"
    )

    if finite_count != total:
        raise RuntimeError("real-router combined output contains non-finite values")
