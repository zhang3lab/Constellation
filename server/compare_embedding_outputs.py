...
def main() -> None:
    ...
    hf_meta = load_json(str(Path(args.hf_dir) / "hf_embedding.json"))
    rt_meta = load_json(str(Path(args.runtime_dir) / "runtime_embedding.json"))

    if hf_meta["input_ids"] != rt_meta["input_ids"]:
        raise RuntimeError("input_ids mismatch")

    hf_tensor = torch.load(Path(args.hf_dir) / "embedding.pt", map_location="cpu")
    rt_tensor = torch.load(Path(args.runtime_dir) / "embedding.pt", map_location="cpu")

    report = {
        "input_ids": hf_meta["input_ids"],
        "comparison": compare_tensors(hf_tensor, rt_tensor, "embedding"),
    }
    ...
    print(f"[compare-embed] wrote {out}")
