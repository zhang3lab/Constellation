...
def main() -> None:
    ...
    with InferenceSession(coord, cfg) as session:
        ...
        q_flash = aux.get("q_flash")
        if q_flash is not None:
            q_flash = torch.from_numpy(q_flash).float().cpu()
            q_nrope_absorb = q_flash[..., :512]
            q_rope = q_flash[..., 512:]
            p1 = outdir / "q_nrope_absorb.pt"
            p2 = outdir / "q_rope.pt"
            torch.save(q_nrope_absorb, p1)
            torch.save(q_rope, p2)
        ...
        with (outdir / "runtime_layer0_qparts.json").open("w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
            f.write("\n")

        print(f"[runtime-qparts] wrote {outdir / 'runtime_layer0_qparts.json'}")
