...
def names_0_to_4(restricted_local_expert_ids: list[int] | None = None) -> list[str]:
...
def main() -> None:
    ...
    print(f"[hf-layer4] restricted_local_expert_ids={restricted_local_expert_ids}")

    loader = DeepseekModelLoader(args.model_dir)
    model = load_hf_model(args.model_dir, args.device)

    try:
        for name in names_0_to_4(restricted_local_expert_ids):
            print(f"[hf-layer4] copy {name}")
        ...
        dbg = getattr(model.model.layers[4], "last_debug", {}) or {}
        router_dbg = getattr(model.model.layers[4].mlp.gate, "last_router_debug", {}) or {}
        ...
        save_tensor_if_present(saved, outdir, dbg, "attention_output", "layer_4_attention_output.pt")
        save_tensor_if_present(saved, outdir, dbg, "post_attention_hidden", "layer_4_post_attention_hidden.pt")
        save_tensor_if_present(saved, outdir, dbg, "ffn_hidden", "layer_4_ffn_hidden.pt")
        save_tensor_if_present(saved, outdir, dbg, "shared_expert_output", "layer_4_shared_expert_output.pt")
        save_tensor_if_present(saved, outdir, dbg, "routed_output", "layer_4_routed_output.pt")
        save_tensor_if_present(saved, outdir, dbg, "ffn_total", "layer_4_ffn_total.pt")
        save_tensor_if_present(saved, outdir, dbg, "layer_output", "layer_4_output.pt")

        save_tensor_if_present(saved, outdir, router_dbg, "logits", "layer_4_logits.pt")
        save_tensor_if_present(saved, outdir, router_dbg, "scores", "layer_4_scores.pt")
        save_tensor_if_present(saved, outdir, router_dbg, "scores_for_choice", "layer_4_scores_for_choice.pt")
        save_tensor_if_present(saved, outdir, router_dbg, "group_scores", "layer_4_group_scores.pt")
        save_tensor_if_present(saved, outdir, router_dbg, "selected_group_idx", "layer_4_selected_group_idx.pt")
        save_tensor_if_present(saved, outdir, router_dbg, "score_mask", "layer_4_score_mask.pt")
        save_tensor_if_present(saved, outdir, router_dbg, "topk_choice_vals", "layer_4_topk_choice_vals.pt")
        save_tensor_if_present(saved, outdir, router_dbg, "topk_idx", "layer_4_topk_idx.pt")
        save_tensor_if_present(saved, outdir, router_dbg, "topk_weight", "layer_4_topk_weight.pt")
        save_tensor_if_present(saved, outdir, router_dbg, "resident_mask", "layer_4_resident_mask.pt")

        save_tensor_if_present(saved, outdir, dbg, "topk_ids", "layer_4_moe_topk_ids.pt")
        save_tensor_if_present(saved, outdir, dbg, "topk_weight", "layer_4_moe_topk_weight.pt")
        save_tensor_if_present(saved, outdir, dbg, "restored_by_token", "layer_4_moe_restored_by_token.pt")
        save_tensor_if_present(saved, outdir, dbg, "weighted_by_token", "layer_4_moe_weighted_by_token.pt")
        save_tensor_if_present(saved, outdir, dbg, "final_out", "layer_4_moe_final_out.pt")
        ...
        with (outdir / "hf_layer4_manual.json").open("w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
            f.write("\n")

        print(f"[hf-layer4] wrote {outdir / 'hf_layer4_manual.json'}")
    finally:
        del model
