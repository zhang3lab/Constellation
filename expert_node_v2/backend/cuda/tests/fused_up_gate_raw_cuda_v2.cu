...
__device__ inline float warp_sum_raw(float v) {
...
__global__ void fused_up_gate_raw_kernel(
    int rows,
    int cols,
    ...
    up_sum = warp_sum_raw(up_sum);
    gate_sum = warp_sum_raw(gate_sum);
...
bool LaunchFusedUpGateRawCudaV2(
    ...
    fused_up_gate_raw_kernel<TAct, kWarpsPerBlock><<<grid, block, 0, stream>>>(
        rows,
        cols,
