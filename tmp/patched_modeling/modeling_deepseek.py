...
class MoEGate(nn.Module):
    def __init__(self, config):
        ...
        self.resident_expert_ids = None
        self.last_router_debug = None
...
class DeepseekV3MoE(nn.Module):
    ...
    def moe_infer(self, x, topk_ids, topk_weight):
        ...
        if self.ep_size > 1:
            ...
            s = 0
            for expert_id, k in enumerate(tokens_per_expert_group.cpu().numpy().tolist()):
                if k > 0:
            ...
            local_keep_mask = np.array(
                [
                    ...
                    for eid in gathered_global_expert_ids
                ],
                dtype=bool,
            )
...
# Patched DeepSeek-V3 attention module that replaces the standard eager attention
# kernel with an absorbed-latent no-cache reference implementation.
class DeepseekV3AbsorbedAttention(nn.Module):
    """DeepSeek-V3 attention implemented with an absorbed-latent reference kernel."""
    ...
    def _absorbed_attention_no_cache(
        self,
        *,
        q_a: torch.Tensor,
        q_nope: torch.Tensor,
        ...
        Args:
          q_a:           [B, T, q_lora_rank]
          q_nope:        [B, T, H, qk_nope_head_dim]
          q_pe:          [B, T, H, qk_rope_head_dim]
          cache_latent:  [B, T, kv_lora_rank]
          cache_k_rope:  [B, T, qk_rope_head_dim]
     
        Returns:
          out:           [B, T, hidden_size]
        """
        bsz, q_len, _ = q_a.shape
        if bsz != 1:
            raise NotImplementedError(
        ...
        q_pe_rot, k_pe_rot = apply_rotary_pos_emb(
            q_pe_hf,
            k_pe_hf,
            cos,
            sin,
        ...
        for t in range(q_len):
            ...
            q_abs_t = q_nope_absorb[0, t].unsqueeze(0)        # [1, H, kv_rank]
            q_rope_t = q_pe_rot[0, :, t, :].unsqueeze(0)      # [1, H, rope_dim]
            ...
            latent_out = torch.einsum(
                ...
                probs,
                latent_prefix.float(),
            ).to(q_abs_t.dtype)  # [1, H, kv_rank]
     
            kv_up_v = kv_b[:, self.qk_nope_head_dim :, :]     # [H, Vdim, kv_rank]
            value_heads = torch.einsum(
                "bhk,hvk->bhv",
                latent_out.float(),
                kv_up_v.float(),
            ).to(latent_out.dtype)  # [1, H, Vdim]
     
            value_flat = value_heads.reshape(1, self.num_heads * self.v_head_dim)
            hidden_t = torch.matmul(
                value_flat.float(),
                self.o_proj.weight.float().t(),
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None or use_cache:
            raise NotImplementedError(
                "DeepseekV3AbsorbedAttention cache path not implemented yet"
            )
        if output_attentions:
            raise NotImplementedError(
                "DeepseekV3AbsorbedAttention does not return attention weights yet"
            )
        ...
        if bsz != 1:
            raise NotImplementedError(
                f"DeepseekV3AbsorbedAttention currently only supports bsz=1, got {bsz}"
            )
        if position_ids is None:
            raise ValueError("position_ids must not be None for absorbed attention")
        if q_len < 1:
            raise ValueError(f"q_len must be >= 1, got {q_len}")
...
ATTENTION_CLASSES = {
    ...
    "absorbed_latent_ref": DeepseekV3AbsorbedLatentRefAttention,
}
